...
filenames = {'U': "/alphadata04/onink/lagrangian_sim/GlobCurrentDailyData/GlobCurrent_Zonal_2*.nc",
             'V': "/alphadata04/onink/lagrangian_sim/GlobCurrentDailyData/GlobCurrent_Meridional_2*.nc"}
variables = {'U': 'uo',
             'V': 'vo'}
...
lons=np.load('/alphadata04/onink/lagrangian_sim/Azores/InputDistribution/LonsAzoresInitialDistribution0_2.npy')
lats=np.load('/alphadata04/onink/lagrangian_sim/Azores/InputDistribution/LatsAzoresInitialDistribution0_2.npy')
...
def AgeSample(particle, fieldset,time,dt):
...
def OldAge(particle,fieldset,time,dt):
    if particle.Age/365>10:
        particle.delete()#remove a particle after 10 years, so all particles get a full 10 year run
Distsam=pset.Kernel(TotalDistance) 
Agesam=pset.Kernel(AgeSample)
oldAge=pset.Kernel(OldAge)
totalKernal=oldAge+AdvectionRK4+Distsam+Agesam
#%%
savestep=24 #save everything ever 2 days
pfile = pset.ParticleFile(name="/alphadata04/onink/lagrangian_sim/Azores/Output/Seasonality/Azores2013Season",
...
pset.execute(totalKernal,
             runtime=timedelta(days=360),  # we only want 11 additional releases, so only 335 days
...
pset.repeatdt=None
#And then we continue the run for 10 more years, without releasing new particles
pset.execute(totalKernal,
             runtime=timedelta(days=365*10),  # runtime controls the interval of the plots
             dt=-timedelta(minutes=30),
             recovery={ErrorCode.ErrorOutOfBounds: DeleteParticle},
             output_file=pfile
             )  # the recovery kernel
