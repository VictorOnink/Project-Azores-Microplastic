...
for k in range(2010,2019):
    for m in range(1,13):
        ...
        for i in range(len(time)):
            timetemp=datetime(1950,1,1,0,0)+timedelta(hours=time[i])
            timetemp=timetemp.replace(minute=0,hour=0)
            azoresEKEtime.append(timetemp)
            azoresEKE.append(np.nanmean(EKE[i,:,:]))
azoresEKEtime=azoresEKEtime

#%% Calculate the weekly running mean average of the EKE
EKEmean=pd.DataFrame({'EKE':azoresEKE})
EKEmean=np.array(EKEmean.rolling(7,win_type=None).mean())
...
for i in range(14):
    for k in range(len(azoresEKEtime)):
        for j in range(len(normEvents)):
            if azoresEKEtime[k]==(normEvents[j]-timedelta(days=i)):
                normEKE[i,j]+=EKEmean[k]
        for j in range(len(specEvents)):
            if azoresEKEtime[k]==(specEvents[j]-timedelta(days=i)): 
                specEKE[i,j]+=EKEmean[k]
